...
class AssetBase(BaseModel):
    ...
    def validate_geofence(self):
        ...
        if self.geofence_latitude is None:
            raise ValueError(
                "Geofence latitude is required when geofencing is enabled."
            )

        if self.geofence_longitude is None:
            raise ValueError(
                "Geofence longitude is required when geofencing is enabled."
            )

        if self.geofence_radius_meters is None:
            raise ValueError(
                "Geofence radius is required when geofencing is enabled."
            )

        if not -90 <= self.geofence_latitude <= 90:
            raise ValueError(
                "Geofence latitude must be between -90 and 90."
            )

        if not -180 <= self.geofence_longitude <= 180:
            raise ValueError(
                "Geofence longitude must be between -180 and 180."
            )

        if self.geofence_radius_meters <= 0:
            raise ValueError(
...
class AssetUpdate(BaseModel):
    ...
    geofence_radius_meters: float | None = Field(
        default=None,
        gt=0,
    )
...
class AssetResponse(AssetBase):
    ...
    tracker_key_created_at: datetime | None = None
    ...
    def from_asset(cls, asset):
        return cls(
            ...
            geofence_enabled=asset.geofence_enabled,
            geofence_latitude=asset.geofence_latitude,
            geofence_longitude=asset.geofence_longitude,
            geofence_radius_meters=asset.geofence_radius_meters,
            notes=asset.notes,
